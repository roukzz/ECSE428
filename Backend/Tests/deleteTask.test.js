...
describe("Delete Task Test", () => {
    ...
    it("should delete a task of an authenticated user", async () => {
        const res1 = await request(app).post('/api/authentication/login').send({
            username: 'student',
            password: 'password'
        });
        const res2 = await request(app).put('/api/student/deleteStudentTask').set("auth-token", res1.headers["auth-token"])
        .send({
            username: 'student',
    ...
    it("should not delete a task of an unauthenticated user", async () => {
        const res = await request(app).put('/api/student/deleteStudentTask').send({
            username: 'student',
            taskId: this.task._id.toString(),
        });
