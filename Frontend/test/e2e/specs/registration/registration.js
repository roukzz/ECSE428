  ...
  'Test valid email and valid password': function (client) {
    ...
    for (var i = 0; i < registrations.length; i++) {
      client
        ...
        .click(config.id.signupbtn)
        .pause(config.time.pause)
        .assert.urlEquals(config.destinationUrl)
        .waitForElementVisible('body', config.time.visible)
        .assert.visible(config.id.accountAndSettings)
        .click(config.id.accountAndSettings)
        .pause(config.time.pause)
        .assert.visible(config.id.profileButton)
        .click(config.id.profileButton)
        .pause(config.time.pause)
        .assert.visible(config.id.deleteAccount)
        .click(config.id.deleteAccount)
        .pause(config.time.pause)
    }
    client
      .end();
